...
<div class="container col-lg-11 form-control">
	...
	$sysCat;
		foreach ($systemData as $key => $value) {
			...
			$sysWidth  = $value->sysWidth;
			$sysHeight  = $value->sysHeight;
			$sysType = $value->sysType;
			$sysCat  = $value->sysCategory;
		}
	?>
	...
	<div class="row">
		...
		<div class="col-lg-6" >
			...
			<input class="form-control" type="text" id="txtSyscode" value="<?php echo $sysCode?>" disabled>
		</div>
		<div class="col-lg-3" >
			<label>System Type</label>
			<select class="form-control" id="txtsysType" disabled>
				<?php 
					foreach ($systypedd as $key => $value) {
						if($value->dropVal == $sysType){
							echo '<option value="'.$value->dropVal.'" Selected>'.$value->dropVal.'</option>';
						}else{
	...
	<div class="row">
		...
		<div class="col-lg-3" >
			...
			<input class="form-control width" type="text" id="txtSyswidth" value="<?php echo $sysWidth?>" disabled>
		</div>	
		<div class="col-lg-3" >
			<label>Height (mm)</label>
			<input class="form-control height" type="text" id="txtSyheight" value="<?php echo $sysHeight?>" disabled>
		</div>
		<div class="col-lg-3" >
			<label>System Category</label>
			<select class="form-control" id="txtSyscat" disabled>
				<?php 
					foreach ($syscatdd as $key => $value) {
						if($value->dropVal == $sysCat){
							echo '<option value="'.$value->dropVal.'" Selected>'.$value->dropVal.'</option>';
						}else{
							echo '<option value="'.$value->dropVal.'">'.$value->dropVal.'</option>';
						}
				...
				<tbody id="compClasslist">
					...
					if(!empty($syscomp)){
						foreach ($syscomp as $key => $value) {
							echo '<tr id="dbd'.$value->sysCompID.'">';
								echo '<td>'.$value->dropName.'</td>';
								echo '<td>'.$value->quantity.'</td>';
								echo '<td>'.$value->attributeDimension.'</td>';
								echo '<td>'.$value->size.'</td>';
								echo '<td>'.$value->sysMat.'</td>';
								echo '<td><button class="btn btn-danger btn-sm fa fa-trash" data-toggle="modal" data-target="#del" onclick="getme('.$value->sysCompID.')"></button></td>';
							echo '</tr>';
						}
					}
					if(!empty($sysmat)){
						foreach ($sysmat as $key => $value) {
							echo '<tr id="dbd'.$value->sysCompID.'">';
...
<div id="del" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
      	<button type="button" class="btn btn-danger" data-dismiss="modal" onclick="deleted()">OK</button>
        <button type="button" class="btn btn-info" data-dismiss="modal">Cancel</button>
      </div>
    </div>
...
function deleted(){
	$.ajax({
	    ...
	    data: {
	    'xmlGet' : subCompclass,
		'xmlRequest' : 'delsyscomp'
		},
	    success: function(data){
	    	 $('#dbd'+subCompclass).remove();
	    }
	});
}
</script>
